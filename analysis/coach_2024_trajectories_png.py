...
def create_2024_trajectories_plot():
    ...
    for coach in coaches_2024:
        ...
        ax.text(coach_df['Season_Number'].iloc[-1] + 0.3, 
               ...
               ha='left', va='center', family='Cambria')
    ...
    ax.text(ax.get_xlim()[1] * 0.98, 2, 'Replacement Level', 
            ha='right', va='bottom', fontsize=10, style='italic', family='Cambria')
    
    # Customize the plot
    ax.set_xlabel('Season Number in Career', fontsize=16, family='Cambria')
    ax.set_ylabel('Cumulative WAR (Wins Above Replacement)', fontsize=16, family='Cambria')
    ax.set_title('2024 NFL Coaches: Career WAR Trajectories', fontsize=18, fontweight='bold', family='Cambria')
    ...
    for i, (_, row) in enumerate(all_indexed_df.iterrows()):
        ...
        if col_idx < 4:  # Safety check
            war_sign = "+" if row['Final_WAR_Games'] >= 0 else ""
            war_value = f"{war_sign}{row['Final_WAR_Games']:.1f}"
            cols[col_idx].append(f"{int(row['Index']):2d}. {row['Coach']:<18} {war_value:>7}")
    ...
    fig.text(ax_center_x, 0.20, legend_text, ha='center', va='top', fontsize=12,
    ...
    # Save the plot
    output_file = 'analysis/coach_2024_trajectories.png'
...
if __name__ == "__main__":
    ...
    data = create_2024_trajectories_plot()
