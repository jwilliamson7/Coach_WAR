"""
3-Year Average WAR Persistence by Background
Compares 3-year average WAR persistence for Offensive vs Defensive coaches.
"""
...
def load_data():
    """Load 3-year average data."""
    print("Loading 3-year average WAR data...")
    df = pd.read_csv('analysis/outputs/csv/coaching_war_3_year_avg_data.csv')
...
def analyze_by_background(df):
    """Analyze 3-year average persistence by background."""
    print("\n" + "="*80)
    print("3-YEAR AVERAGE WAR PERSISTENCE BY BACKGROUND")
    print("="*80)
    ...
    for background in ['Offensive', 'Defensive']:
        ...
        print(f"  Persistence: {model.coef_[0]*100:.1f}% of 3-year avg persists to Year N+1")
...
def create_background_comparison_scatter(results):
    """Create scatter plot comparing Offensive vs Defensive 3-year persistence."""
    print("\nCreating 3-year average background comparison scatter plot...")
    ...
    ax.set_xlabel('3-Year Average Coaching WAR (Games per Season)', fontsize=13, fontweight='bold')
    ax.set_ylabel('Year N+1 Coaching WAR (Games per Season)', fontsize=13, fontweight='bold')
    ax.set_title('3-Year Average WAR Persistence by Background', fontsize=14, fontweight='bold', pad=20)
    ...
    for background in ['Offensive', 'Defensive']:
        ...
        stats_text += f"\n{background}:\n"
        stats_text += f"  $WAR_{{N+1}} = {coef:.3f} \\cdot WAR_{{3yr}} + {intercept:.2f}$ games\n"
    ...
    plt.tight_layout()
    plt.savefig('analysis/outputs/png/coaching_war_3year_persistence_by_background.png',
...
def main(font_family='serif'):
    ...
    print("SUMMARY")
    print("="*80)
    print(f"\nOffensive Coaches (3-year avg): R² = {results['Offensive']['r2']:.4f}")
    print(f"Defensive Coaches (3-year avg): R² = {results['Defensive']['r2']:.4f}")
    r2_diff = results['Defensive']['r2'] - results['Offensive']['r2']
    pct_change = (r2_diff / results['Offensive']['r2']) * 100
    ...
    if r2_diff > 0:
        print("\nInterpretation:")
        print("  - Defensive coaches show stronger 3-year persistence")
    ...
    print("\nResults saved to:")
    print("  - analysis/outputs/png/coaching_war_3year_persistence_by_background.png")
